                        ...
                        <table id="results-table" class="table table-striped table-light table-hover">
                            <thead>
                                <tr>
                                    ...
                                    <th scope="col" id="sort_EventName" data-label="EventName" class="col-md-0">Event Name</th>
                                    <th scope="col" id="sort_EventLastHeartbeat" data-label="EventLastHeartbeat" class="col-md-0">Last Heartbeat</th>
                                    <th scope="col" id="sort_FlgInactive" data-label="FlgInactive" class="col-md-0">Active?</th>
                                    <th scope="col" id="sort_SOURCES" data-label="SOURCES" class="col-md-0">Sources</th>
                                    <th scope="col" id="sort_HTML" data-label="HTML" class="col-md-0">HTM</th>
                                    <th scope="col" id="sort_EXCEL" data-label="EXCEL" class="col-md-0">XLS</th>
                                    <th scope="col" id="sort_POWERPOINT" data-label="POWERPOINT" class="col-md-0">PPT</th>
                                </tr>
                            </thead>
                                ...
                                <tbody class="results-body">
                                    ...
                                    For i=1 To rs.RecordCount
	                                    If Not rs.EOF Then 
                                            ...
                                            ExcelHtml = ""
                                            PowerpointHtml = ""
                                            If rs("flgInactive") = 0 Then FlgInactiveHtml = ""
                                            If rs("HTML") > 0 Then HtmHtml = "<i class='bi bi-filetype-html'> x" & Cstr(rs("HTML")) & "</i>"
                                            If rs("EXCEL") > 0 Then ExcelHtml = "<i class='bi bi-file-excel text-success'> x" & Cstr(rs("EXCEL")) & "</i>"
                                            If rs("POWERPOINT") > 0 Then PowerpointHtml = "<i class='bi bi-file-ppt text-primary'> x" & Cstr(rs("POWERPOINT")) & "</i>"

                                            %>
                                            <tr id="<%=rs("ID") %>">
                                                <th scope="row"><%=rs("ID") %></th>
                                                <td class="text-nowrap"><%=rs("EventName") %></td>
                                                <td class="text-nowrap"><%=FormatDateTime(rs("EventLastHeartbeat"),vbShortDate) %>&nbsp;<%=FormatDateTime(rs("EventLastHeartbeat"),vbShortTime) %></td>
                                                <td class="text-nowrap"><%=FlgInactiveHtml %></td>
                                                <td class="text-nowrap"><%=SourcesHtml %></td>
                                                <td class="text-nowrap"><%=HtmHtml %></td>
                                                <td class="text-nowrap"><%=ExcelHtml %></td>
                                                <td class="text-nowrap"><%=PowerpointHtml %></td>
                                            </tr>
                                            <%
                                            rs.MoveNext
                                        End If
